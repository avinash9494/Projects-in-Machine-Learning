...
def calculation(data):
	...
	mean=copy.deepcopy(cluster)
	rows=len(data)
	cols=len(data[0])
	data_ch=[item for item in data if item not in cluster]
	data_c=data_ch[:]
#	data_copy1=data_ch[:]
	dist=[[0 for row in range(0,k)]for col in range(0,len(data))]
	r=-1
	c=0
	c1=0
	r1=0
	...
	for i in range(0,len(dist),1):
		...
		for j in range(0,len(dist[0]),1):
			...
			while(f==1):
				while(flag==1):
					dist[i][j]+=data[r][c]-cluster[j][c1]
					c=c+1
					if(c==len(data[0])):
						c=0
						flag=0
	...
	while(k<=len(cluster)):
		...
		while(i<len(data)):
			i=i+1
			while(i<len(data) and data[i][j-1]==k):
	...
	cluster=copy.deepcopy(mean)
	print("\n")
	
	d_c=[[0 for row in range(0,len(data[0]))]for col in range(0,len(data))]
	j=0
	k=len(data[0])-1
	while(j<len(data[0])):
